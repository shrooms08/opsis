...
import { resolveConfig, DEFAULT_RPC_URL } from '../src/config.js';
const base = {
    signature: 'sig',
    json: false,
    rpcUrl: undefined,
...
check('flag wins over env', resolveConfig({ ...base, rpcUrl: 'http://flag.example' }, { OPSIS_RPC_URL: 'http://env.example' }), { ok: true, config: { rpcUrl: 'http://flag.example', idlDir: undefined, fixtureDir: './fixtures', requestTimeoutMs: 10_000 } });
check('env used when no flag', resolveConfig(base, { OPSIS_RPC_URL: 'http://env.example:8899' }).ok &&
    resolveConfig(base, { OPSIS_RPC_URL: 'http://env.example:8899' }), { ok: true, config: { rpcUrl: 'http://env.example:8899', idlDir: undefined, fixtureDir: './fixtures', requestTimeoutMs: 10_000 } });
