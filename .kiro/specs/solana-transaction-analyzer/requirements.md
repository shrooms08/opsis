...
- **Anchor_Attestation**: Evidence in the transaction response that the program which raised an error is built with the Anchor framework. Exactly two forms qualify: an AnchorError log line in the verbatim log messages whose reported error number equals the transaction's error code, or a loaded Anchor_IDL whose program ID equals the failing instruction's program ID. Membership of the error code in a numeric range is not attestation.
- **Error_Resolver**: Component that maps error codes to human-readable error messages
- **Error_Attestation**: The recorded basis on which a resolved error's namespace was established — one of program-id, anchor-error-log, or idl
...
1. WHEN an error code value is greater than or equal to 6000 AND Anchor_Attestation exists for the failing instruction's program, THE Error_Resolver SHALL resolve the error code against that program's Anchor_IDL by matching the error code to the IDL errors array
2. WHEN an error code value is in the range 2000 to 5999 inclusive AND Anchor_Attestation exists for the failing instruction's program, THE Error_Resolver SHALL resolve the error code against the Anchor framework error table, where membership of the error code in that range is one of two required conditions and does not on its own license the lookup
...
5. WHEN an error code value is greater than or equal to 6000 AND Anchor_Attestation exists for the failing instruction's program AND no Anchor_IDL is loaded for that program, THE Error_Resolver SHALL include the numeric error code with Confidence_Marker "raw", no error message, and an unresolved reason value of no-idl in the Analysis_Object
6. WHEN no error table governs the error code, because the failing instruction's program is not System Program, SPL Token, or SPL Associated Token Account, no Anchor_Attestation exists for the failing instruction's program, and the error code value falls outside every numeric range that a framework error table uses, THE Error_Resolver SHALL include the numeric error code with Confidence_Marker "raw", no error message, and an unresolved reason value of not-in-table in the Analysis_Object, which is a condition distinct from the case in criterion 10 where an error table does govern the error code but does not contain it, and distinct from the case in criterion 11 where a framework error table's numeric range covers the error code but no Anchor_Attestation exists
7. THE Error_Resolver SHALL include the Confidence_Marker field with value "full" when the error namespace was established by Anchor_Attestation or by program ID equality against a known program, or with value "raw" when no basis for the error namespace was established
...
10. WHEN an error table governs the error code, because the failing instruction's program is System Program, SPL Token, or SPL Associated Token Account, or because Anchor_Attestation exists for the failing instruction's program and the table selected per criterion 1 or criterion 2 is available, AND the error code is absent from that error table, THE Error_Resolver SHALL include the numeric error code with Confidence_Marker "raw", no error message, and an unresolved reason value of not-in-table in the Analysis_Object, which is a condition distinct from the case in criterion 6 where no error table governs the error code at all, distinct from the case in criterion 11 where a framework error table's numeric range covers the error code but no Anchor_Attestation exists, and distinct from the case in criterion 5 where Anchor_Attestation exists for an error code value greater than or equal to 6000 but no Anchor_IDL is loaded and therefore no table governs the error code
11. IF an error code value falls within a numeric range that a framework error table uses AND no Anchor_Attestation exists for the failing instruction's program AND the failing instruction's program is not System Program, SPL Token, or SPL Associated Token Account, THEN THE Error_Resolver SHALL include the numeric error code with Confidence_Marker "raw", no error message, and an unresolved reason value of unattested-namespace in the Analysis_Object, which is a condition distinct from the case in criterion 6 where no error table governs the error code and the error code value falls outside every framework error table's numeric range, and distinct from the case in criterion 10 where an error table governs the error code but does not contain it
12. WHEN error resolution succeeds, THE Error_Resolver SHALL record an Error_Attestation value of program-id, anchor-error-log, or idl in the Analysis_Object identifying what established the error namespace
13. WHERE Anchor_Attestation is an AnchorError log line, THE Error_Resolver SHALL take the error name and the error message verbatim from that AnchorError log line rather than from an error table, and SHALL record the Error_Attestation value as anchor-error-log
14. THE Error_Resolver SHALL resolve System Program, SPL Token, and SPL Associated Token Account error codes without requiring Anchor_Attestation and SHALL record the Error_Attestation value for those resolutions as program-id, because those three programs are identified by fixed program IDs and their identity is therefore established by address equality rather than inferred
15. WHEN an AnchorError log line appears in the verbatim log messages AND the error number reported in that AnchorError log line equals the transaction's error code, THE Error_Resolver SHALL treat that AnchorError log line as Anchor_Attestation for the failing instruction's program
16. WHEN a loaded Anchor_IDL declares a program ID equal to the failing instruction's program ID, THE Error_Resolver SHALL treat that Anchor_IDL as Anchor_Attestation for the failing instruction's program
17. IF the verbatim log messages contain no AnchorError log line whose reported error number equals the transaction's error code AND no loaded Anchor_IDL declares a program ID equal to the failing instruction's program ID, THEN THE Error_Resolver SHALL treat Anchor_Attestation as absent for the failing instruction's program
